...
<template>
  <v-toolbar
    ...
    <v-spacer></v-spacer>
      <v-text-field
        class="centered-input text--darken-3 mt-2"
        append-icon="search"
        label="Regular"
        color="white"
        :label="$t('IP адрес, ЯААТ номи...')">
      </v-text-field>
    <v-spacer></v-spacer>
    <v-dialog
      v-model="dialog"
...
export default {
  ...
  methods: {
    logout(){
      const vm = this;
      window.location.href='http://localhost:8080/realms/spring-project-v1-realm/protocol/openid-connect/logout'
    },
    onLogoutClicked() {
